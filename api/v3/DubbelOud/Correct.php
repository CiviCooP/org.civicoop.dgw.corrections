...
/**
 * DubbelOud.Correct API
 * incident BOSW1507030
 *
 * @param array $params
 ...
 */
function civicrm_api3_dubbel_oud_correct($params) {
  $sameContactId = null;
  $keepAddress = array();
  $countRemoved = 0;
  ...
  $queryParams = array(1 => array(6, 'Integer'));
  ...
  while ($dao->fetch()) {
    ...
    if ($dao->address_id != $keepAddress['address_id']) {
      checkAddress($dao->address_id, $keepAddress, $countRemoved);
    }
  }
  $returnValues = array('is_error'=> 0, 'message' => 'Verwerking succesvol afgerond, '.$countRemoved.' addressen verwijderd.');
  return civicrm_api3_create_success($returnValues, $params, 'DubbelOud', 'Correct');
}

function checkAddress($addressId, $keepAddress, &$countRemoved) {
  try {
